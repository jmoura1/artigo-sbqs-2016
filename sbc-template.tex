...
%Outra dificuldade: como um processo pode ser executado por diferentes usuários, os cenários também são, então os metódos java teram que executar a troca de usuário quando necessário para executar a tarefa do cenário (fazer login, logout e login ....): identificar usuários e prever
...
Executando o parser criado utilizando o arquivo BPMN referente ao diagrama da Figura \ref{fig:diagrama_exemplo} como exemplo, obtemos os cenários de teste representados na Figura \ref{fig:cenario_resultado}. Nestes cenários...
%como foi mencionado, apenas tarefas executadas por usuários são levadas para o teste funcional

Para cada etapa nos cenários criados será criado um método correspondente dentro do arquivo de \emph{steps} chamado \emph{stepDefinition}. Por exemplo, para a etapa X será criado um método com um nome genérico e contendo a anotação Given X. Os métodos poderão ser preenchidos posteriormente com os dados gravados pelo Selenium na etapa de captura da interação com a aplicação.
...
%falar e colocar a tabelinhas, alem dos dois arquivos consegue extrair a tabelinha
...
Pode se dizer que este trabalho atingiu seu objetivo...
